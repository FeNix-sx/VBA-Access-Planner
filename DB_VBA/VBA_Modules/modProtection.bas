...
'################################################################
Public Function AdvancedHash(inputText As String) As String
    On Error GoTo ErrorHandler
...
'################################################################
'########        ПРОЦЕДУРА УСТАНОВКИ/СМЕНЫ ПАРОЛЯ        ########
'################################################################
Public Sub SetAdminPassword()
    ...
    newPassword = InputBox("Введите новый пароль администратова: ", "Установка пароля")
    
    ' ЕСЛИ ОТМЕНА ИЛИ ПУСТАЯ СТРОКА
    If newPassword = "" Then
    ...
    confirm = MsgBox("Вы уверены, что хотите установить этот пароль?" & vbCrLf & _
                     "Запомните его - восстановление будет невозможно." & vbCrLf & vbCrLf & _
                     "Новый пароль будет сохранен в зашифрованном виде.", _
    ...
    ' хэшируем пароль
    passwordHash = AdvancedHash(newPassword)
    
    ' сохраняем ХЭШ в базу данных
    Call SaveLicenseSetting("AdminPassword", passwordHash)
    
    MsgBox "Пароль администратора успешно установлен!", vbInformation
    
    Debug.Print "Новый хэш пароля: " & passwordHash
End Sub

'################################################################
'########            ВКЛЮЧЕНИЕ КЛАВИШИ SHIFT             ########
'################################################################
Public Sub EnableShiftKey()
    ...
    ' включаем возможность обхода с помощью SHIFT
    db.Properties("AllowByPassKey") = True
    
    MsgBox "Клавиша Shift успешно включена!", vbInformation, "Защита выключена"
    Exit Sub
...
ErrorHandler:
    ...
    If Err.Number = PROPERTY_NOT_FOUND Then
        ...
        Resume Next
    Else
        MsgBox "Ошибка включения клавиши Shift: " & Err.Number & _
               " | " & Err.description
    End If
End Sub

'################################################################
'########            ОТКЛЮЧЕНИЕ КЛАВИШИ SHIFT            ########
'################################################################
Public Sub DisableShiftKey()
    ...
    MsgBox "Клавиша Shift успешно отключена!", vbExclamation, "Защита активирована"
    Exit Sub

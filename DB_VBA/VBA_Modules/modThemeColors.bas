...
'################################################################
'########    Палитра активной темы из БД (без формы)     ########
'################################################################
Public Sub Theme_LoadPaletteFromDatabase()
' Назначение: Заполняет глобальную палитру (Theme_WritePalette) по строке
'             tbThemes с IsActive=True, иначе первой теме по ThemeID.
' Принцип:    Только чтение DAO; без обновления флагов и без UI календаря.
'================================================================
...
Err_Handler:
    ...
    Set rs = Nothing
    Debug.Print "[Theme_LoadPaletteFromDatabase] " & Err.Number & " " & Err.description
End Sub
